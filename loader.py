...
class DataConverter:
    ...
    def data_convert(self) -> None:
        ...
        self.excluded_tickers = self._get_invalid_tickers(
            data_section, tickers, data_types)

        if self.excluded_tickers:
            excluded_path = os.path.join(
                self.output_dir, "excluded_tickers.json")
            with open(excluded_path, 'w', encoding='utf-8') as f:
                json.dump(list(self.excluded_tickers), f,
                          ensure_ascii=False, indent=2)
            print(
                f"Excluded tickers due to #INVALID OPTION: {sorted(self.excluded_tickers)}")

        for data_type in ["open", "high", "low", "close", "volume"]:
            type_df = self._get_data_by_type(
                data_section, tickers, data_types, data_type)
            if not type_df.empty:
                type_df.index.name = None
                out_path = os.path.join(
                    self.output_dir, f"{data_type}.parquet")
...
class DataLoader:
    ...
    def _register(self) -> Dict[str, 'DataLoader.DataEntry']:
        files = [f for f in os.listdir(
            self.data_dir) if f.endswith('.parquet')]
        return {
