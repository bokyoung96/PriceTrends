...
class Windows:
    ...
    def save(self, path: Path) -> Path:
        ...
        torch.save({
            "data": self.data,
            "targets_cls": self.targets_cls,
            "targets_dd": self.targets_dd,
            "dates": self.dates,
            "assets": self.assets,
    ...
    def load(path: Path) -> "Windows":
        ...
        targets_cls = d.get("targets_cls", d.get("targets"))
        targets_dd = d.get("targets_dd", np.zeros_like(targets_cls, dtype=np.float32))
        return Windows(d["data"], targets_cls, targets_dd, d["dates"], d["assets"])
...
class WindowMaker(IWindowMaker):
    ...
    def make(self, panel: Panel, frames: Dict[str, pd.DataFrame]) -> Windows:
        ...
        targets_cls, targets_dd, d_list, a_list = [], [], [], []
        counter = 0
        ...
        for s in tqdm(range(0, max_start + 1, self.cfg.stride), desc="Creating windows"):
            ...
            fut_slice = close_vals[t_idx:t_idx + self.cfg.horizon + 1]
            fut_slice_ok = np.isfinite(fut_slice).all(axis=0)

            ok = (counts >= min_valid) & fut_ok & fut_slice_ok
            idx = np.where(ok)[0]
            ...
            t_cls = fut[idx]
            price_slice = fut_slice[:, idx]
            rel = price_slice / price_slice[0]
            run_max = np.maximum.accumulate(rel, axis=0)
            dd = (run_max - rel) / (run_max + 1e-8)
            dd_mag = np.max(dd, axis=0)

            targets_cls.append(t_cls)
            targets_dd.append(dd_mag.astype(np.float32))
        ...
        t_cls = np.concatenate(targets_cls, axis=0).astype(np.float32)
        t_dd = np.concatenate(targets_dd, axis=0).astype(np.float32)
        ...
        if self.cfg.label_type == "classification":
            t_cls = (t_cls > self.cfg.threshold).astype(np.float32)

        return Windows(d, t_cls, t_dd, dt, at)
...
class StockDataset(TorchDataset):
    def __init__(self, w: Windows):
        self.d = w.data
        self.t_cls = w.targets_cls
        self.t_dd = w.targets_dd
        self.dt = w.dates
        self.a = w.assets
    ...
    def __getitem__(self, i):
        return {
            ...
            "label": torch.tensor(int(self.t_cls[i]), dtype=torch.long),
            "dd": torch.tensor(self.t_dd[i], dtype=torch.float32),
            "date": str(self.dt[i]),
            "asset": self.a[i]
        }
